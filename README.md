...
class Board:
    ...
    def get_board(self):
        board_str = "   |  " + "  |  ".join(str(item) for item in range(8)) + "  | " + "  | ".join(str(item) for item in range(8, 9)) + " |"
    ...
    def add_premium_squares(self):
        TRIPLE_WORD_SCORE = ((0,0), (8,0), (0,8), (8,8))
        DOUBLE_WORD_SCORE = ((1,1), (7,1), (1,7), (7,7))
        TRIPLE_LETTER_SCORE = ((3,0), (5,0), (0,3), (0,5), (3,8), (5,8), (8,3), (8,5))
        DOUBLE_LETTER_SCORE = ((3,3), (5,3), (1,4), (4,1), (7,4), (3,5), (5,5), (4,7))
...
class Word:
    ...
    def check_word(self):
        ...
        if self.word != "":
            ...
            if self.location[0] > 8 or self.location[1] > 8 or self.location[0] < 0 or self.location[1] < 0 or (self.direction == "down" and (self.location[0]+len(self.word)-1) > 8) or (self.direction == "right" and (self.location[1]+len(self.word)-1) > 8):
                return "Location out of bounds.\n"

            #Ensures that first turn of the game will have the word placed at (7,7).
            if round_number == 1 and players[0] == self.player and self.location != [4,4]:
                return "The first turn must begin at location (4, 4).\n"
            return True
...
def turn(player, board, bag):
    ...
    if (skipped_turns < 6) or (player.rack.get_rack_length() == 0 and bag.get_remaining_tiles() == 0):
        ...
        word_to_play = input("Input word: ")
        location = []
        col = input("Column number: ")
        row = input("Row number: ")
        if (col == "" or row == "") or (col not in [str(x) for x in range(9)] or row not in [str(x) for x in range(9)]):
            location = [-1, -1]
        else:
